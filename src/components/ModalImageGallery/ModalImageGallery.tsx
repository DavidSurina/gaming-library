...
import {ChevronLeft as ChevronLeftIcon, ChevronRight as ChevronRightIcon} from "react-bootstrap-icons";
...
function ModalImageGallery(props: PropTypes) {
    ...
    const rightBtnCondition = images.results && imageIndex !== images.results.length - 1;
    ...
    return (
        <Modal show={modalOpen} onHide={resetGallery} fullscreen>
            ...
            <Modal.Body>
                <Carousel
                    fade activeIndex={imageIndex} interval={null}
                    ...
                    <div className="img-gallery_wrapper" ref={scrollWrapperRef}>
                        {
                            images.results.map((result, index) => (
                                <div key={result.id}
                                     className={`${imageIndex === index ? ' image_selected' : ''} img-gallery_img-wrapper`}
